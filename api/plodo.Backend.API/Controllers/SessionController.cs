...
public class SessionController : ControllerBase
{
    ...
    public async Task<ActionResult> Delete(string sessionId)
    {
        ...
        await _serverSentEventsService.SendEventAsync(sessionId, new ServerSentEvent {Type = "terminate", Data = [""
            ]
        });
    ...
    public async Task<ActionResult> Vote(string sessionId, CastVoteRequest request)
    {
        ...
        try
        {
            var audienceId = Guid.Parse(HttpContext.User.FindFirst("audience_id")?.Value ?? throw new InvalidOperationException());
            ...
            await _serverSentEventsService.SendEventAsync(sessionId,
                new ServerSentEvent {Type = "vote", Data = [request.Vote.ToLower()] });
